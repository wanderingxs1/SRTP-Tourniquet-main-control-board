...
void TM1640_start()
{ 
	HAL_GPIO_WritePin(TM1640_GPIOPORT,DISPDIN ,GPIO_PIN_SET ); //接口输出高电平1	
	HAL_GPIO_WritePin(TM1640_GPIOPORT,DISPSCLK,GPIO_PIN_SET ); //接口输出高电平1	
	delay_us(DEL);
	HAL_GPIO_WritePin(TM1640_GPIOPORT,DISPDIN,GPIO_PIN_RESET ); //接口输出0	
	delay_us(DEL);
	HAL_GPIO_WritePin(TM1640_GPIOPORT,DISPSCLK,GPIO_PIN_RESET ); //接口输出0	
	delay_us(DEL);
}

void TM1640_stop()
{
	HAL_GPIO_WritePin(TM1640_GPIOPORT,DISPDIN,GPIO_PIN_RESET );
	HAL_GPIO_WritePin(TM1640_GPIOPORT,DISPSCLK,GPIO_PIN_SET );
	delay_us(DEL);
	HAL_GPIO_WritePin(TM1640_GPIOPORT,DISPDIN,GPIO_PIN_SET ); 
	delay_us(DEL);
}
...
{
	const u8 buff[21]={0xEB,0x88,0xB3,0xBA,0xD8,0x7A,0x7B,0xA8,0xFB,0xFA,0x00};//数字0~9及0~9加点显示段码表
